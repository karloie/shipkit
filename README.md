...
**Outputs:** `latest_tag`, `next_tag`, `publish`, `dryrun`, `release_tag`, `version`, `version_major_minor`, `dockerfile`, `summary_message`, `goreleaser_config_current`, `goreleaser_config_auto`, `goreleaser_docker`
...
- Detects `Containerfile.goreleaser` or `Dockerfile.goreleaser` for Docker publishing (when present, GoReleaser handles Docker builds and `docker-publish.yml` workflow is skipped)
...
- `.github/workflows/docker-build.yml` - Docker build (build without push)
- `.github/workflows/docker-publish.yml` - Docker publishing (build and push)
- `.github/workflows/go-build.yml` - Go build and test
- `.github/workflows/go-publish.yml` - Go publishing (GoReleaser)
- `.github/workflows/maven-build.yml` - Maven build and test
- `.github/workflows/maven-publish.yml` - Maven publishing
- `.github/workflows/npm-build.yml` - NPM build and test
...
- `node_version` (optional) - Node version for frontend build in go-publish job (default: none)
...
- `node_version` (optional): Node version for frontend build in go-publish job (e.g. `22`)

### Workflow Notes

- **Docker publishing:** If `Containerfile.goreleaser` or `Dockerfile.goreleaser` exists, GoReleaser handles Docker builds and `docker-publish.yml` is skipped. Otherwise, `release.yml` calls `docker-build.yml` during the build phase and `docker-publish.yml` during the publish phase.
