...
func TestGetProjectPattern(t *testing.T) {
	tests := []struct {
		...
		{"found go", "Go", true, "Go"},
		{"found nodejs", "Node.js", true, "Node.js"},
...
func TestDetectProjectTypesWithLogging(t *testing.T) {
	// This test verifies that detectProjectTypesWithLogging works correctly
	// with both logging enabled and disabled

	// Test with logging disabled - should not panic
	detected := detectProjectTypesWithLogging(false)
	// Verify it returns a usable slice (nil is ok in Go for empty slices)
	_ = len(detected) // Should not panic

	// Test with logging enabled - should not panic
	detectedWithLog := detectProjectTypesWithLogging(true)
	_ = len(detectedWithLog) // Should not panic

	// Both should return the same results regardless of logging
...
func TestDetectDockerFiles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Just verify return types are valid
			if found && fileName == "" {
...
func TestDetectProjectTypesCaching(t *testing.T) {
	// Verify that detectProjectTypes caches results across multiple calls
	first := detectProjectTypes()
	second := detectProjectTypes()
	third := detectProjectTypes()

	// All calls should return the same cached slice
	...
	// Verify they contain the same project types
