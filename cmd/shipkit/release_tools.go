...
// runReleaseGoreleaser executes goreleaser to publish releases
func runReleaseGoreleaser(args []string) error {
	...
	skipPublish := fs.Bool("skip-release", false, "Skip releasing")
	...
	logInputs(map[string]string{
		...
		"skip-release": fmt.Sprintf("%v", *skipPublish),
	...
	if *skipPublish {
		cmdArgs = append(cmdArgs, "--skip-release")
	}
	...
	fmt.Fprintf(os.Stderr, "✅ GoReleaser release completed successfully\n")
...
// runReleaseDocker builds and publishes Docker images
func runReleaseDocker(args []string) error {
	// Log raw args BEFORE parsing
	logInputs(map[string]string{
		"raw_args": strings.Join(args, " "),
	})
	...
	fmt.Fprintf(os.Stderr, "✅ Docker release completed successfully\n")
